...
export async function fetchDashboard(range, signal) {
  ...
  return {
    ...
    ministeres: safeArray(data?.ministeres).map((item) => ({
      ...
      soumissionsPayees: Number(item.soumissionsPayees || 0),
...
export async function fetchSoumissions(params = {}, signal) {
  ...
  return {
    donnees: safeArray(data?.donnees || data).map(item => ({
      ...
      service: typeof item.service === 'object' ? (item.service?.nomFr || item.service?.nom || '') : (item.service || ''),
      serviceId: typeof item.service === 'object' ? item.service?.id : item.serviceId,
      ministere: typeof item.ministere === 'object' ? (item.ministere?.nomFr || item.ministere?.nom || '') : (item.ministere || ''),
      ministereId: typeof item.ministere === 'object' ? item.ministere?.id : item.ministereId,
      domaine: typeof item.domaine === 'object' ? (item.domaine?.nomFr || item.domaine?.nom || '') : (item.domaine || ''),
      orgUnit: typeof item.orgUnit === 'object' ? (item.orgUnit?.nomFr || item.orgUnit?.nom || '') : (item.orgUnit || ''),
